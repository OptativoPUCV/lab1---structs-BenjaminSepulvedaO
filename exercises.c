...
void mergeSortedArrays(int arr1[], int size1, int arr2[], int size2,
  ...
  while (i < size1 && j < size2)
    {
      if (arr1[i] <= arr2[j] && arr1[i] > numAnterior)
      {
        result[k] = arr1[i];
        i++;
      }
      else if (arr1[i] > arr2[j] && arr2[j] > numAnterior)
      {
        result[k] = arr2[j];
        j++;
      }
      /*else if (arr1[i] == arr2[j] && arr1[i] > numAnterior)
      {
        result[k] = arr1[i];
        i++;
        j++;
      }*/
      numAnterior = result[k];
      k++;
    }
  while (i < size1)
    {
...
int checkSorted(int arr[], int size) { 
  ...
  while (i < size)
    {
      if (arr[i] < arr[i - 1] && orden == 1) return 0;
      else if (arr[i] > arr[i - 1] && orden == -1) return 0;
      i++;
    }
  return orden; 
}
